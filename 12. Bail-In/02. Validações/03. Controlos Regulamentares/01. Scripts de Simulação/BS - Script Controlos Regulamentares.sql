...
--	,CASE WHEN (`5000_flg_shrhldr_not_rsltn_grp` IS MISSING or cast(`5000_flg_shrhldr_not_rsltn_grp` as string) = '') AND B99.`0060_orgnl_amnt_fx` = 'Non-Resolution Entity' THEN 1 ELSE 0 END AS v_1_0039
    ...
    ,CASE WHEN `0130_rlvnt_amnt_wdc` != GREATEST(0, `0120_otstndng_amnt`-`0300_amnt_cvrd_elgbl_dpsts`-`0320_amnt_clltrl`) AND `0300_amnt_cvrd_elgbl_dpsts`!= 999999999999999.00 AND `0320_amnt_clltrl`!= 999999999999999.00 AND `0250_strctrd_prdct` IN ('Non-structured/Vanilla', 'Other non-standard terms') AND `0240_zero_coupon` != 'ZCB issued at discount' THEN 1 ELSE 0 END AS v_2_0011
...
FROM 
	...
	,CASE WHEN `0070_flg_scrd` IN ('Secured', 'Unsecured') THEN 0 ELSE 1 END AS v_2_0056
