...
FROM (
	SELECT 
		...
		CASE --CNTRPRTY_RSLTN_GRP: campo novo (6/11 não concluído)
			WHEN AUX_CNTRPRTY_RSLTN_GRP.FLG_INTRGRP = '1' AND AUX_CNTRPRTY_RSLTN_GRP.FLG_INTR_RSLTN_GRP = '1' THEN 'Intragroup and intra-resolution group'
			WHEN AUX_CNTRPRTY_RSLTN_GRP.FLG_INTRGRP = '1' AND AUX_CNTRPRTY_RSLTN_GRP.FLG_INTR_RSLTN_GRP = '0' THEN 'Intragroup but not intra-resolution group'
			WHEN AUX_CNTRPRTY_RSLTN_GRP.FLG_INTRGRP = '0' THEN 'No'
			ELSE 'Not applicable'
		...
		,CASE -- campo já existente (6/11 não concluído) - CAMPO OK NA LT
			...
			WHEN CT003.ENTTY_NM IS NULL	THEN 'Not applicable'
			ELSE CT003.ENTTY_NM
		END AS ENTTY_NM 
		 
		,CASE
			WHEN CT003.CLEI IN ('','#') AND AUX_1A.CLEI_GRUPO_AUX1A IS NOT NULL THEN AUX_1A.CLEI_GRUPO_AUX1A
		    WHEN CT003.CLEI IS NULL OR TRIM(CT003.CLEI)='' THEN ''
		    WHEN CT003.CLEI = '#' THEN 'Not applicable'
		    ELSE CT003.CLEI
		END AS LEI
		...
		,CASE 
			...
			WHEN CT003.CPAIS_RESIDENCIA IS NULL		THEN '99'
	...
	LEFT JOIN
		...
		) AUX_CNTRPRTY_RSLTN_GRP
		ON ZCLIENTE = UNIV.ENTTY_ID
