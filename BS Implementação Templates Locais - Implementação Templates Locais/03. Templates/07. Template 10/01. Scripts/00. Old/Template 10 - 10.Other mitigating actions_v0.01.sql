...
(
	...
	FROM BU_ESG_WORK.P3_FULL_CTR_CLI_LOCAL_DEZ24_V5
	WHERE TEMPLATE10=1  
    AND DT_RFRNC = '${REF_DATE}'
...
DROP TABLE BU_ESG_WORK.LOCAL_PILAR3_TEMP10_DEZ24_FINAL;	
CREATE TABLE BU_ESG_WORK.LOCAL_PILAR3_TEMP10_DEZ24_FINAL AS
SELECT 
    CASE 
        WHEN TRIM(UPPER(XX.INSTRUMENTO_FINANCEIRO)) = 'CREDITO CONCEDIDO E OUTROS ATIVOS FINANCEIROS' THEN 'LOANS'  
        WHEN TRIM(UPPER(XX.INSTRUMENTO_FINANCEIRO))= 'INSTRUMENTOS DE DIVIDA' THEN 'BONDS'
    END AS CONTA,	
    X.CONTRAPARTE, 
    X.TIPO_COLATERAL,
