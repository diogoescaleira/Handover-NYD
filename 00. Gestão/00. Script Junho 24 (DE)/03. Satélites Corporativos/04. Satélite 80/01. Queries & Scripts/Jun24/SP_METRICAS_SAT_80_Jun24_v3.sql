...
-- 393 205 
-- 393 218 jun24 (dados dez23)
-- 25.406.239.798 --> Nova corrida Dez23 
-- loans and advances
--drop table bu_esg_work.rf_pilar3_tabaux_sat80_jun24;
--create table bu_esg_work.rf_pilar3_tabaux_sat80_jun24 as
select distinct
...
from (  select distinct 
            ...
            cnumecta_ct,
            zdeposit_ct
		from bu_esg_work.RF_PILAR3_UNIVERSO_FULL where csatelite = 80 and idcomb_satelite not like '%MC10%' and DT_RFRNC = '${ref_date}' and ID_CORRIDA = '3') as a 
...
;
-- 162
-- 163 jun24 (dados dez23)
-- tabela final
--drop table bu_esg_work.rf_pilar3_satelite80_jun24;
--create table bu_esg_work.rf_pilar3_satelite80_jun24 as
select 
    reporting_soc,
    case 
...
case 
        ...
        when ep_score =  'EPSC7' and epc_label <> 'EPCL8' then concat(idcomb_satelite,';','EPSD3',';',ep_score,';',epc_label_data,';',epc_label)-- alterado para jun24
        when ep_score <> 'EPSC7' and epc_label =  'EPCL8' then concat(idcomb_satelite,';',ep_score_data,';',ep_score,';','EPCD3',';',epc_label)-- alterado para jun24
...
from bu_esg_work.rf_pilar3_tabaux_sat80_jun24
group by 1,2,3,4,6
;

-- 505
-- 506 jun24 (dados dez23)
-- tangible assets
--drop table bu_esg_work.rf_pilar3_tabaux80adj_jun24;
--create table bu_esg_work.rf_pilar3_tabaux80adj_jun24 as
select 
        a.idcomb_satelite, 
...
from (  select csatelite, idcomb_satelite, sociedade_contraparte, cod_ajust, cargabal_ct, sum(saldo_ct) as saldo_ct 
		from bu_esg_work.RF_PILAR3_UNIVERSO_FULL where csatelite = 80 and idcomb_satelite like '%MC10%' and DT_RFRNC = '${ref_date}' and id_corrida='3'
...
-- 24
-- 24 jun24 (dados dez23) 
-- tabela final
--drop table bu_esg_work.rf_pilar3_satelite80adj_jun24;
--create table bu_esg_work.rf_pilar3_satelite80adj_jun24 as
select 
    '00411' as reporting_soc,
    case 
        when sociedade_contraparte = '' then '00000'
...
from bu_esg_work.rf_pilar3_tabaux80adj_jun24
group by 1,2,3,4,6
;
...
--drop table bu_esg_work.rf_pilar3_satelite80v0_jun24;
--create table bu_esg_work.rf_pilar3_satelite80v0_jun24 as
select * from bu_esg_work.rf_pilar3_satelite80_jun24
union
select * from bu_esg_work.rf_pilar3_satelite80adj_jun24
;

-- Desconsideração de operações SC02 --> Por indicação da corporação (Cargabal Masterizado sem visibilidade de SC02+COLL3)
-- 179 registos
-- 180 jun24 (dados dez)
--drop table bu_esg_work.rf_pilar3_satelite80_final_jun24;
--create table bu_esg_work.rf_pilar3_satelite80_final_jun24 as
Select * from bu_esg_work.rf_pilar3_satelite80v0_jun24
...
Select * 
from bu_esg_work.rf_pilar3_satelite80v0_jun24
where id_comb not like '%SC02%'
...
-- 26 436 715 109
select sum(saldo_ct)
from bu_esg_work.rf_pilar3_universo_full_jun24 where csatelite = 80 and idcomb_satelite not like '%MC10%'
--from bu_esg_work.RF_PILAR3_UNIVERSO_FULL_jun24 where csatelite = 80 and idcomb_satelite not like '%MC10%' and DT_RFRNC = '${ref_date}' -- descomentar em junho
