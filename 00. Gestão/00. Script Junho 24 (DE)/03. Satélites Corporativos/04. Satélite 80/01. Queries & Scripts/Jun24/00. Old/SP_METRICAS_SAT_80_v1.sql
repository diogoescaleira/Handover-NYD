...
from (  select distinct 
            ...
            cnumecta_ct,
            zdeposit_ct
		from bu_esg_work.RF_PILAR3_UNIVERSO_FULL where csatelite = 80 and idcomb_satelite not like '%MC10%' and DT_RFRNC = '${ref_date}') as a 
...
(
	SELECT DISTINCT 
	    ...
	    GAR.cnumecta_ct,
	    GAR.zdeposit_ct,
	    nvl(GAR.`13_gross_carrying_amount`,0) as `13_gross_carrying_amount`,
...
from (  select csatelite, idcomb_satelite, sociedade_contraparte, cod_ajust, cargabal_ct, sum(saldo_ct) as saldo_ct 
		from bu_esg_work.RF_PILAR3_UNIVERSO_FULL where csatelite = 80 and idcomb_satelite like '%MC10%' and DT_RFRNC = '${ref_date}' 
		
        group by 1,2,3,4,5) as a
        
        left join 
        
        (select a.*, fiabilidad, clase_energetica,emisiones,consumos from
            (select * from bu_esg_work.adjudicados_dez23_final
            where tipo_adjudicado <> 'Totta URBE' and cargabal_vc in ('1605000','1605010')) a
            left join 
            (SELECT *FROM bu_esg_work.gloval_clase_energetica_dez23 
            ) CERT_ENERG
	    ON cod_imovel = chave_banco_atual )b
...
Select * 
from bu_esg_work.rf_pilar3_satelite80_final_dez23 -- alterar tabela dependendo do rácio
...
select sum(amount) from bu_esg_work.RT_PILAR3_SATELITE80 where DT_RFRNC = '${ref_date}' and PROC_ID 
in (select max(PROC_ID) from bu_esg_work.RT_PILAR3_SATELITE80 where DT_RFRNC = '${ref_date}');
...
-- 25 406 239 816.56
select sum(saldo_ct)
from bu_esg_work.rf_pilar3_universo_full where DT_RFRNC = '${ref_date}' and csatelite = 80 and idcomb_satelite not like '%MC10%'
