...
-- Ficheiro de Emissões disponibilizado pela corporação CIB: bu_esg_work.pilar3_avg_emission_cibjun24
-- Ficheiro de Emissões disponibilizado pela corporação Revenues&Assets: bu_esg_work.pilar3_avg_emission_rev_assetjun24
-- Ficheiro de Emissões disponibilizado pela corporação Real Estate: bu_esg_work.pilar3_avg_emission_realstatejun24
...
-- Tabela de controlo de Gestão bu_esg_work.rf_pilar3_cdg_tabaux1jun24 ==> Deverá ser corrido este procedimento 
...
-- Identificação do Calculation Approach  
					case when concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053','000Y45') and cdg.cnumecta_ct is not null then 'Motor vehicle loans' -- Apenas marcamos MVL que estão na tabela de CdG porque apenas temos tipo_viatura deles
			...
			-- A) SCORE 2: Estimado quando é possível obter o valor de emissões na sheet "Vehicle Loans - PCAF factor" de marca/modelo para país Portugal -- SEM INFO REPRESENTATIVA A DEZ23
 			-- B) SCORE 3: Estimado quando não é possível obter o valor de emissões na sheet "Vehicle Loans - PCAF factor" de marca/modelo para país Portugal -- SEM INFO REPRESENTATIVA A DEZ23
  			...
  			-- C) SCORE 4: Desconhecida marca/modelo é usada uma proxy pelo tipo de veículo e country
					...
					avg_km.amount as amount_score4_km,
    				...
    				avg_emiss_elect_hib.amount_scope1 as elec_avg_motor_vehicle_scope1, -- Montante da Emissão do ficheiro da corporação
    				avg_emiss_elect_hib.amount_scope2 as elec_avg_motor_vehicle_scope2, -- Montante da Emissão do ficheiro da corporação					
  
 	-- APENAS HYBRID PLUG-IN PODEM CALCULAR EMISSOES SCOPE1, NESTE CASO NÃO EXISTE NO BANCO FORMA DE IDENFITICAR OS PLUG-IN, POR ISSO SÃO FORÇADOS A CALCULAR APENAS SCOPE2
 					case 
						when setor = 'NFC' and avg_emiss_veh.type is not null and tipo_combustivel not in ('Hybrid','Electric')
								...
								and (case when concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053','000Y45') and cdg.cnumecta_ct is not null then 'Motor vehicle loans' 
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then avg_emiss_veh.amount_scope1 * avg_km.amount 
						
						when setor = 'NFC' and tipo_viatura = 'Passenger car' and tipo_combustivel in ('Electric')
								-- Excluir casos de Project Finance e Business Loans - Identificar apenas calculation approach = Motor vehicle loans 
								and (case when concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053','000Y45') and cdg.cnumecta_ct is not null then 'Motor vehicle loans' 
										  when `29_flag_specialised_lending` = '1' then 'Project Finance'
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then avg_emiss_elect_hib.amount_scope1 * avg_km.amount 														
						else null end as emi_veh_score4_scope1,
						
						when setor = 'NFC' and tipo_viatura = 'Passenger car' and tipo_combustivel in ('Hybrid')
								-- Excluir casos de Project Finance e Business Loans - Identificar apenas calculation approach = Motor vehicle loans 
								and (case when concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053','000Y45') and cdg.cnumecta_ct is not null then 'Motor vehicle loans' 
										  when `29_flag_specialised_lending` = '1' then 'Project Finance'
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
								...
								and (case when concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053','000Y45') and cdg.cnumecta_ct is not null then 'Motor vehicle loans' 
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then avg_emiss_veh.amount_scope2 * avg_km.amount 
							
						when setor = 'NFC' and tipo_viatura = 'Passenger car' and tipo_combustivel in ('Hybrid','Electric')
								-- Excluir casos de Project Finance e Business Loans - Identificar apenas calculation approach = Motor vehicle loans 
								and (case when concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053','000Y45') and cdg.cnumecta_ct is not null then 'Motor vehicle loans' 
										  when `29_flag_specialised_lending` = '1' then 'Project Finance'
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then avg_emiss_elect_hib.amount_scope2 * avg_km.amount 							
  			...
  			-- D) SCORE 5: Desconhecida marca/modelo é usada uma proxy apenas para Passenger car e apenas com disinção de elétrico e hibrido
					...
					avg_emiss_veh_score5.amount_scope1 as amount_veh_score5_scope1, -- Montante da Emissão do ficheiro da corporação
					avg_emiss_veh_score5.amount_scope2 as amount_veh_score5_scope2, -- Montante da Emissão do ficheiro da corporação
					avg_km_score5.amount as amount_score5_km,
  
 					case 
						when setor = 'NFC' and tipo_viatura = '' and  avg_emiss_veh.type is null --and tipo_combustivel not in ('Hybrid','Electric')
								-- Excluir casos de Project Finance e Business Loans - Identificar apenas calculation approach = Motor vehicle loans 
								and (case when concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053','000Y45') and cdg.cnumecta_ct is not null then 'Motor vehicle loans' 
										  when `29_flag_specialised_lending` = '1' then 'Project Finance'
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then avg_emiss_veh_score5.amount_scope1 * avg_km_score5.amount
								...
								and (case when concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053','000Y45') and cdg.cnumecta_ct is not null then 'Motor vehicle loans' 
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then avg_emiss_veh_score5.amount_scope2 * avg_km_score5.amount							
					...
					case when (avg_emiss_veh.type is not null or avg_emiss_elect_hib.type is not null) then '4' end as dqs4_motor_vehicle_loans,
					case when  avg_emiss_veh.type is null and avg_emiss_veh_score5.type is not null then '5' end as dqs5_motor_vehicle_loans
...
from bu_esg_work.pilar3_avg_emission_rev_assetjun24 where functional_unit = 'Revenue') c   
...
from bu_esg_work.pilar3_avg_emission_rev_assetjun24 where functional_unit = 'Assets') d   
...
( 
	...
	--LEI --1
		...
		from
			(
			select 'Y' as CIB, * from bu_esg_work.pilar3_avg_emission_cibjun24
			) corp
		inner join 
...
left join 
	(select  	cempresa_ct,
				...
				ckctabem,
				ckrefbem
		from bu_esg_work.reparticao_garantias_final_Jun24_v2
...
left join 
	(select distinct 
					...
					des_combustivel,
					co2,
					case when ckprodmi in ('000045','000051','000053','000Y45') and des_detalle_lease  = 'VEICULOS PESADOS DE PASSAGEIROS' then 'Bus'
						 when ckprodmi in ('000045','000051','000053','000Y45') and des_detalle_lease in ('COMERCIAIS ATE 1600KG','COMERCIAIS DE 1601KG A 2600KG','COMERCIAIS DE 2601KG A 3500KG') then 'Light Commercial Truck'
						 when ckprodmi in ('000045','000051','000053','000Y45') and des_detalle_lease  = 'VEICULOS PESADOS DE MERCADORIAS' then 'Medium/Heavy Commercial Truck'
						 when ckprodmi in ('000045','000051','000053','000Y45') and des_detalle_lease = 'VEICULOS LIGEIROS DE PASSAGEIROS' then 'Passenger car'
						 when ckprodmi in ('000045','000051','000053','000Y45') and des_detalle_lease = 'MOTOCICLOS CILINDRADA SUPERIOR A 50CM3' then 'Motorcycle' 
						 when ckprodmi in ('000045','000051','000053','000Y45') and des_detalle_lease like 'CICLOMOTOR%' then 'Motorcycle' 						 
						 when ckprodmi in ('000045','000051','000053','000Y45') and des_detalle_lease  = 'REBOQUE/SEMI-REBOQUE C/MATRICULA' then 'Medium/Heavy Commercial Truck'
						 when ckprodmi in ('000045','000051','000053','000Y45') and des_detalle_lease  = 'EMPILHADOR' then 'Medium/Heavy Commercial Truck'
						 when ckprodmi in ('000045','000051','000053','000Y45') and des_detalle_lease in ('TT ATE 1600KG','TT DE 1601KG A 2600KG','TT DE 2601KG A 3500KG') then 'Passenger car'
					else ''
					end as tipo_viatura,
					case when ckprodmi in ('000045','000051','000053','000Y45') then 'Passenger car' else '' end as tipo_viatura_score5,					
					case
						 when ckprodmi in ('000045','000051','000053','000Y45') and des_combustivel in ('HIBRIDO','HIBRIDO/GASOLINA','HIBRIDO/GASOLEO') then 'Hybrid'
						 when ckprodmi in ('000045','000051','000053','000Y45') and des_combustivel = 'ELECTRICO' then 'Electric'
...
left join            
         (   select distinct 
			    ...
			    from  
        			(   select * 
        			    from bu_esg_work.pilar3_avg_emission_rev_assetjun24 a 
        			    left join  
        			        (   select *,
...
left join (select * from bu_esg_work.avg_km_Jun24) avg_km
on cdg.tipo_viatura = avg_km.type 

left join (select * from bu_esg_work.avg_emis_fact_veh_Jun24) avg_emiss_veh
on cdg.tipo_viatura = avg_emiss_veh.type 

left join (select * from bu_esg_work.avg_emis_veh_hib_elec_Jun24) avg_emiss_elect_hib
on cdg.tipo_combustivel = avg_emiss_elect_hib.type 

-- AUXILIAR DE PROXY PARA SCORE5 QUANDO NÃO TEM TIPO_VIATURA FORÇAMOS A QUE SEJA 'PASSAGER CAR' E 'THERMAL')
left join (select * from bu_esg_work.avg_emis_fact_veh_Jun24) avg_emiss_veh_score5
on cdg.tipo_viatura_score5 = avg_emiss_veh_score5.type 

-- AUXILIAR DE PROXY PARA KMS QUANDO NÃO TEM TIPO_VIATURA FORÇAMOS A QUE SEJA 'PASSAGER CAR' E 'THERMAL')
left join (select * from bu_esg_work.avg_km_Jun24) avg_km_score5
on cdg.tipo_viatura_score5 = avg_km_score5.type 
;
...
/*-- drop table if exists bu_esg_work.pilar3_sat93_tabaux2_Jun24_1_aux;
create table bu_esg_work.pilar3_sat93_tabaux2_Jun24_1_aux as  
select  
        t1.reporting_soc,
...
(
    select *
    from bu_esg_work.pilar3_sat93_tabaux2_Jun24_1
) t1
left join
(
    select *
...
union all select * from bu_esg_work.pilar3_sat93_tabaux2_Jun24_1_aux*/
...
-- Cálculo de Emissões para Real Estate 
    ...
    -- 2º Método: Estimated building emissions based on floor area
            -- A) Emissions based on official building energy labels and the floor area 
                    ...
                    -- Scope 2
                    case when setor = 'NFC' and t2.ckctabem is not null and t2.narutil is not null and t2.narutil > 0 and `29_flag_specialised_lending`<>'1' and `29_flag_specialised_lending`<>'1'  
                    									and trim(clase_energetica) <>'' and clase_energetica is not null and fiabilidad in ('SANTANDER','','1-REAL')
                                                        -- and cnael = 'CNAEL11' -- NACE de Real Estate Activities     
...
-- Cruzamento para quando é sabido o tipo de edificío (apenas office à data)
        left join ( 
                    select *
                    from bu_esg_work.pilar3_avg_emission_realstatejun24
...
insert overwrite table  bu_esg_work.RT_PILAR3_SATELITE93_Jun24 partition (DT_RFRNC, PROC_ID)
