...
create table bu_esg_work.pilar3_sat93_tabaux1_jun24_1 as
select 
...
from (																				
	...
	inner join (select * from bu_esg_work.p3_ctr_cli
				where DT_RFRNC = '${ref_date}'
				and id_corrida in 
			 (select max(id_corrida) from bu_esg_work.p3_ctr_cli where DT_RFRNC = '${ref_date}') --- !! atenção: validar se é necessário usar particionada ou versão final
	) a	-- Tabela métricas clientes + contrato	
...
create table bu_esg_work.pilar3_sat93_tabaux1_jun24 as
select t2.*, 
		case
...
from bu_esg_work.pilar3_sat93_tabaux1_jun24_1 as t2
...
create table bu_esg_work.pilar3_sat93_tabaux2_jun24_1 as  
				...
				--Cálculo de emissões, scope 1 e 2, quando sabemos o tipo de viatura e o tipo de combustível
 					case 
						when (a.amount > 0 and valor_financeiro > 0) and setor = 'NFC' and avg_emiss_veh.data_level_1_information is not null and tipo_combustivel is not null
							...
							then (NVL(avg_emiss_veh.scope_1_emission_factor,0) * avg_km.average_km) * ((a.amount/1000000)/valor_financeiro) --Casos em que o carro é electrico as emissões de scope 1 estão a null - AC
								...
								and (case when ((concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053') and cdg.cnumecta_ct is not null)
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then NVL(avg_emiss_veh.scope_2_emission_factor,0) * avg_km.average_km * ((a.amount/1000000)/valor_financeiro) -- Casos em que o carro é a combustão as emissões de scope 2 estão a null - AC
								...
								and (case when ((concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053') and cdg.cnumecta_ct is not null)
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 
							then avg_emiss_veh_score5_type.scope_1_emission_factor * avg_km_score5_type.average_km * ((a.amount/1000000)/valor_financeiro)
								...
								and (case when ((concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053') and cdg.cnumecta_ct is not null)
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then avg_emiss_veh_score5_proxy.scope_1_emission_factor * avg_km_score5_proxy.average_km * ((a.amount/1000000)/valor_financeiro)
...
left join 
	(select distinct 
            ...
            from bu_esg_work.rf_pilar3_cdg_tabaux1_jun24_v2	
...
create table bu_esg_work.pilar3_sat93_tabaux2_jun24_1_aux2 as
select t1.* from bu_esg_work.pilar3_sat93_tabaux2_jun24_1 t1
/*left join
(
    select *
    from bu_esg_work.reparticao_garantias_final_dez23
...
-- Dez23        ==> 310 411  registos 
--              ==> 12 330 703 151.672943 €
drop table if exists bu_esg_work.pilar3_sat93_tabaux2_jun24_2;
create table bu_esg_work.pilar3_sat93_tabaux2_jun24_2 as  
select *,
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
from bu_esg_work.pilar3_sat93_tabaux2_jun24_1_aux2;
...
create table bu_esg_work.pilar3_sat93_tabaux2_jun24 as 
select *,
...
from bu_esg_work.pilar3_sat93_tabaux2_jun24_2;
...
create table bu_esg_work.pilar3_sat93_tabaux_emi_contrato_jun24 as 
select  *,
...
from bu_esg_work.pilar3_sat93_tabaux2_jun24;
...
create table bu_esg_work.pilar3_sat93_tabaux3_AUX_jun24_aux_1 as
                   ...
                   -- 3º Método: Estimated building emissions based on number of buildings                     
                    ...
                    case 
						...
						when setor = 'NFC' and t2.ckctabem is not null and `29_flag_specialised_lending`<>'1' 
									...
									end as emi_scope2_dwelling_inten,                   
...
-- DATA QUALITY SCORE									
					...
					real_estate_Residential.pcaf_dqs as dqs_Residential,
                    real_estate_others.pcaf_dqs as dqs_others,                                   
                    real_estate_total_floor.pcaf_dqs as dqs_totalfloor,
                    real_estate_total_emitensity_office.pcaf_dqs  as dqs_emintensity_office,
                    real_estate_total_emitensity_Residential.pcaf_dqs as dqs_emintensity_residential,
...
left join
            ( 
                select *
                from bu_esg_work.gpt32_bemimove_jun24 --- IMPORTANTE: CORRER ESTA TABELA NO FINAL DO MÊS DO RÁCIO
            ) b 
...
-- Cruzamento para aferir o número de buildings/bens        
    left join
        ...
        (
            ...
            from bu_esg_work.reparticao_garantias_final_jun24 as a
...
-- Cruzamento para não sabemos o tipo de edifício e o factor name é Emission Intensity per # + Dwelling é Residencial Real Estate nem na Europa nem na América do Norte
     ...
     left join 
            ...
            , coalesce(ckrefbem, '0') as ckrefbem_key	
                ...
                from bu_esg_work.rf_pilar3_pesos_jun24) as peso
...
create table bu_esg_work.pilar3_sat93_tabaux3_AUX_jun24_aux_2 as
select *, 
...
from bu_esg_work.pilar3_sat93_tabaux3_AUX_jun24_aux_1;
...
create table bu_esg_work.pilar3_sat93_tabaux3_AUX_jun24_aux as
select *, 
...
from  bu_esg_work.pilar3_sat93_tabaux3_AUX_jun24_aux_2;
...
create table bu_esg_work.pilar3_sat93_tabaux3_AUX_jun24 as
select *,
		-- Flag de Calculation Approach
        case when ckctabem is not null then  1 else 0 end as flg_calculation_approach_2
from bu_esg_work.pilar3_sat93_tabaux3_AUX_jun24_aux;
...
create table bu_esg_work.pilar3_sat93_tabaux_emi_colat_aux_jun24 as 
select  *,
        -- emissões finais scope 1
        case 
		...
		'' as dqs_scope3_gar
from bu_esg_work.pilar3_sat93_tabaux3_AUX_jun24;
...
create table bu_esg_work.pilar3_sat93_tabaux_emi_colat_jun24 as 
...
from bu_esg_work.pilar3_sat93_tabaux_emi_colat_aux_jun24
...
create table bu_esg_work.pilar3_sat93_tabaux3_jun24 as
select 
        t2.*,
...
from bu_esg_work.pilar3_sat93_tabaux_emi_colat_jun24 as t2
	left join bu_esg_work.nace_esg_pillar3 as nace_esg								
			on concat(split_part(t2.`15_nace_esg`,".",1),split_part(t2.`15_nace_esg`,".",2),split_part(t2.`15_nace_esg`,".",3)) = trim(split_part(nace_esg.nace_level4, "-",1));
...
create table bu_esg_work.sat93_agregacao_jun24_aux as
select 
		reporting_soc,
		sociedade_contraparte,
		case when sociedade_contraparte = '' then '00000' else sociedade_contraparte end as counterparty_soc,
...
create table bu_esg_work.sat93_agregacao_jun24_aux2 as
select distinct 
        t1.reporting_soc,
        t1.sociedade_contraparte,
        t1.cod_ajust,
