...
from (																				
	...
	inner join (select * from bu_esg_work.p3_ctr_cli_v2) a	-- Tabela métricas clientes + contrato	
		     ...
		     -- A) CIB 
					...
					case 
						when setor = 'NFC' and GLCS_code is not null and (coalesce(cast(total_debt as decimal (30,6)),0) + coalesce(cast(total_equity as decimal (30,6)),0)) > 0 and a.amount > 0 and gar.cnumecta_ct is null 
								...
								and (case when concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053','000Y45') and cdg.cnumecta_ct is not null then 'Motor vehicle loans' 
										  when `29_flag_specialised_lending` = '1' then 'Project Finance'
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
													or
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
			 ...
			 -- B) ASSETS					
					...
					case 
						when (a.amount > 0 and setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1') 
						         and 
						      (case when concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053','000Y45') and cdg.cnumecta_ct is not null then 'Motor vehicle loans' 
										  when `29_flag_specialised_lending` = '1' then 'Project Finance'
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
													or
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
...
-- Cálculo de Emissões para Motor Vehicle Loans  
			...
			-- A) SCORE 2: Estimado quando é possível obter o valor de emissões na sheet "Vehicle Loans - PCAF factor" de marca/modelo para país Portugal -- SEM INFO REPRESENTATIVA A DEZ23 e Jun24
 			-- B) SCORE 3: Estimado quando não é possível obter o valor de emissões na sheet "Vehicle Loans - PCAF factor" de marca/modelo para país Portugal -- SEM INFO REPRESENTATIVA A DEZ23 e Jun24
  			...
  			-- C) SCORE 4: Desconhecida marca/modelo é usada uma proxy pelo tipo de veículo e country
				...
				--Cálculo de emissões, scope 1 e 2, quando sabemos o tipo de viatura e o tipo de combustível
 					case 
						when setor = 'NFC' and avg_emiss_veh.data_level_1_information is not null and tipo_combustivel is not null
								...
								and (case when ((concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053') and cdg.cnumecta_ct is not null)
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then NVL(avg_emiss_veh.scope_1_emission_factor,0) * avg_km.average_km --Casos em que o carro é electrico as emissões de scope 1 estão a null - AC
								...
								and (case when ((concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053') and cdg.cnumecta_ct is not null)
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then NVL(avg_emiss_veh.scope_2_emission_factor,0) * avg_km.average_km -- Casos em que o carro é a combustão as emissões de scope 2 estão a null - AC
  			...
  			-- D) SCORE 5: Desconhecida marca/modelo é usada uma proxy apenas para Passenger car e apenas com disinção de elétrico e hibrido
				...
				-- Cálculo de emissões (Scope 1) quando apenas sabemos o tipo de viatura (Metodologia introduzida em Junho 2024 para DQA 5) - AC
					case
						when setor = 'NFC' and avg_emiss_veh.data_level_1_information is not null and tipo_combustivel is null
						-- Excluir casos de Project Finance e Business Loans - Identificar apenas calculation approach = Motor vehicle loans 
								and (case when ((concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053') and cdg.cnumecta_ct is not null)
													or 
							  					(concat(ct004.cproduto,ct004.csubprod) in ('000042')  and cdg.cnumecta_ct is not null and sfcs_green_activity_comgloval like 'Transport%')) then 'Motor vehicle loans'
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 
							then avg_emiss_veh_score5_type.scope_1_emission_factor * avg_km_score5_type.average_km
				...
				-- Cálculo de emissões (Scope 1) quando não sabemos o tipo de viatura (Usamos o proxy Passenger Car e Average para consigar mapear a distância e emissões) 
						when setor = 'NFC' and tipo_viatura = '' and  avg_emiss_veh.data_level_1_information is null --and tipo_combustivel not in ('Hybrid','Electric')
								-- Excluir casos de Project Finance e Business Loans - Identificar apenas calculation approach = Motor vehicle loans 
								and (case when ((concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053') and cdg.cnumecta_ct is not null)
													or 
							  					(concat(ct004.cproduto,ct004.csubprod) in ('000042')  and cdg.cnumecta_ct is not null and sfcs_green_activity_comgloval like 'Transport%')) then 'Motor vehicle loans' 
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then avg_emiss_veh_score5_proxy.scope_1_emission_factor * avg_km_score5_proxy.average_km
								...
								and (case when ((concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053') and cdg.cnumecta_ct is not null)
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 
							then avg_emiss_veh_score5_type.scope_2_emission_factor * avg_km_score5_type.average_km
								...
								and (case when ((concat(ct004.cproduto,ct004.csubprod) in ('000045','000051','000053') and cdg.cnumecta_ct is not null)
										  ...
										  when ((setor = 'NFC' and GLCS_code is not null and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')
											   ...
											   (setor = 'NFC' and gar.cnumecta_ct is null and `29_flag_specialised_lending`<>'1')) then 'Business Loans'
												...
												and `29_flag_specialised_lending`<>'1'
								-- Cálculo de Emissões 																									
							then avg_emiss_veh_score5_proxy.scope_2_emission_factor * avg_km_score5_proxy.average_km						
...
left join 
	(select  	cempresa_ct,
				...
				ckctabem,
				ckrefbem
		from bu_esg_work.reparticao_garantias_final_dez23_v2
...
left join 
	(select distinct 
					...
					des_combustivel,
					co2,
					case when ckprodmi in ('000045','000051','000053','000042') and des_detalle_lease = 'VEICULOS PESADOS DE PASSAGEIROS' then 'Bus'
					...
					case when ckprodmi in ('000045','000051','000053') then 'Passenger car' else '' end as tipo_viatura_score5,					
					case
...
ON
    case 
        when cdg.tipo_viatura in ('Motorcycle', 'Heavy Vehicle')
            then 'Passenger car' 
    else cdg.tipo_viatura end = avg_km.data_level_1_information 
    ...
    -- 2º Método: Estimated building emissions based on floor area
            -- A) Emissions based on official building energy labels and the floor area 
                    -- Ir buscar tudo o que tenha um colateral, seja ele residencial ou comercial (real estate collaterals) + sabemos o tipo de building
																				-- Data Quality 3
                    -- Scope 1 
							--Escritório/Habitação (junção do mesmo case when para as duas categorias)
                    ...
                    case
						when setor = 'NFC' and t2.ckctabem is not null and t2.narutil is not null and t2.narutil > 0 
														and `29_flag_specialised_lending`<>'1' and `29_flag_specialised_lending`<>'1'  
                    									and trim(clase_energetica) <>'' and clase_energetica is not null and fiabilidad in ('SANTANDER','','1-REAL')
                                                        -- and cnael = 'CNAEL11' -- NACE de Real Estate Activities     
                                                        and trim(pais.cfinbem) = '03' 
                                                        and real_estate_office.data_lvl2_info is not null
...
-- Cruzamento para aferir o número de buildings/bens        
    left join
        ...
        (
            ...
            from bu_esg_work.reparticao_garantias_final_dez23 as a
...
-- Registos a Dezembro2023: 312 744  registos
--                        ==> 14 511 898 490.562943 € 
drop table if exists bu_esg_work.sat93_agregacao2_dez23_v2;
create table bu_esg_work.sat93_agregacao2_dez23_v2 as
select *, 
...
drop table if exists bu_esg_work.pilar3_sat93_tabaux4_dez23_v2;
create table bu_esg_work.pilar3_sat93_tabaux4_dez23_v2 as
select 
        a.reporting_soc,
        a.counterparty_soc,
...
from bu_esg_work.sat93_agregacao2_dez23_v2 as a 
group by 		
		a.reporting_soc,
        a.counterparty_soc,
		a.cod_ajust,
...
-- Registos a Junho2023: 2 028 registos
-- Registos a Dezembro2023: 2 589 registos
drop table bu_esg_work.pilar3_sat93_tabaux5_dez23_v2;
create table bu_esg_work.pilar3_sat93_tabaux5_dez23_v2 as 
select 
        reporting_soc,
        counterparty_soc,
        adjustment_code,
...
from bu_esg_work.pilar3_sat93_tabaux4_dez23_v2;
...
from bu_esg_work.pilar3_sat93_tabaux5_dez23_v2
