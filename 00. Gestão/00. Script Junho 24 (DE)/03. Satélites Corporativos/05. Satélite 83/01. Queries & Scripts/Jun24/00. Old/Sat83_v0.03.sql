...
-- Dez23        ==> -53 399 237 818,14 € (Universo Full s/ MC10)
--              ==>  1 787 304 registos 
--              ==> -53 399 235 600.52 (perdemos 2 217€ devido à tabela de pesos)
...
(select
    ...
    from bu_esg_work.rf_pilar3_universo_full                   -- ALTERADA TABELA PARA NOVA TABELA DE IDCOMBS DEZ23
        where DT_RFRNC = '${ref_date}' and ID_CORRIDA = '2' and csatelite = 83       
            group by
                sociedade_contraparte, 
                idcomb_satelite,
                cempresa_ct, 
...
select  distinct
    ...
    case
        ...
        when  idcomb_satelite like '%SC0304%' and des_combustivel = 'ELECTRICO' and cast(co2 as int) >= 50 then 'GFI1'
    	...
    	when  idcomb_satelite like '%SC0304%' and sfcs_green_activity_comgloval like 'Transport%' then 'GFI2'         
        ...
        when sfcs_green_activity_comgloval = 'Real estate - Building renovation loans' and concat(cproduto,csubprod) in ('096HGP','0960H8','096065') then 'GFI1'
        when sfcs_green_activity_comgloval = 'Real estate - Building renovation loans' and trim(nifrru)<>'' then 'GFI1'
		when idcomb_satelite like '%SC0303%' and sfcs_green_activity_comgloval like 'Real estate%' then 'GFI1'
	-- Building Acquisition Loans 
        when 
          ...
          (
            ...
            (clase_energetica like 'A%' or clase_energetica = 'B') 
         then 'GFI1' 
        when 
            (
                (`33_counterparty_type` = 'particulares' and cfamilia in ('02','03','04') and idcomb_satelite like '%COLL2%' -- FOI FORÇADO A SEREM APENAS COLL2 PORQUE CORPORAÇÃO APENAS PERMITE QUE ACQUISITION SEJA ASSOCIADO A COLL2
          ...
          (
            ...
            (anoconst >= '2020' and anoconst is not null and anoconst<>'9999') and 
            -- Possui certificado energético real           
            fiabilidad in ('','1-REAL','SANTANDER') and 
        
            -- Possui classe energética A ou B -- ALTERAÇÃO GUIDELINES DEZ23     
            (clase_energetica like 'A%' ) 
         then 'GFI1' 
        when 
            (
                (`33_counterparty_type` = 'particulares' and cfamilia in ('02','03','04') and idcomb_satelite like '%COLL2%' -- FOI FORÇADO A SEREM APENAS COLL2 PORQUE CORPORAÇÃO APENAS PERMITE QUE ACQUISITION SEJA ASSOCIADO A COLL2
    ...
    case
     ...
     -- Building Renovation  
        ...
        when idcomb_satelite like '%SC0304%' and sfcs_green_activity_comgloval = 'Real estate - Building renovation loans' and concat(cproduto,csubprod) in ('096HGP','0960H8','096065') then 'PESG1'
        when idcomb_satelite like '%SC0304%' and sfcs_green_activity_comgloval = 'Real estate - Building renovation loans' and trim(nifrru)<>'' then 'PESG1'   
            
     -- Motor Vehicle Loans    
        
        when  idcomb_satelite like '%SC0304%' and des_combustivel = 'ELECTRICO' and cast(co2 as int) >= 50 then 'PESG2'
    	...
    	when  idcomb_satelite like '%SC0304%' and sfcs_green_activity_comgloval like 'Transport%' then '' 
     ...
     -- Building Acquisition Loans 
        ...
        when idcomb_satelite like '%SC0303%' and  sfcs_green_activity_comgloval like 'Real estate%' then ''                    	
     ...
     -- Building Renovation 
        ...
        when sfcs_green_activity_comgloval = 'Real estate - Building renovation loans' and concat(cproduto,csubprod) in ('096HGP','0960H8','096065') then 'FUTY4'
        when sfcs_green_activity_comgloval = 'Real estate - Building renovation loans' and trim(nifrru)<>'' then 'FUTY4'          
     ...
     -- Motor Vehicle Loans    
        ...
        when  idcomb_satelite like '%SC0304%' and des_combustivel = 'ELECTRICO' and cast(co2 as int) >= 50 then 'FUTY4'
        -- -	Gas and plug-in hybrid cars than pollute more than 50g CO2 per km
        when  idcomb_satelite like '%SC0304%' and (
                --(des_combustivel in ('HIBRIDO') and co2 > 50 and co2 <> '' and co2 is not null)  --- > De acordo com o email da Rute Ferreira recebido no dia 4/01 não existe informação nas tabelas de Lease que permita identificar plug-in
                --or
    	...
    	when  idcomb_satelite like '%SC0304%' and sfcs_green_activity_comgloval like 'Transport%' then '' 
			...
			when(		
				case when 
						...
						(
							case when idcomb_satelite like '%SC0304%' and sfcs_green_activity_comgloval = 'Real estate - Building renovation loans' and trim(nifrru)<>'' then 'PESG1'
								 when idcomb_satelite like '%SC0304%' and sfcs_green_activity_comgloval = 'Real estate - Building renovation loans' and concat(cproduto,csubprod) in ('096HGP', '0960H8', '096065')  then 'PESG1'
							end
						  ) <> 'PESG1' and
        	...
        	when idcomb_satelite like '%SC0303%' and  sfcs_green_activity_comgloval like 'Real estate%' then 'FUTY4' 
     ...
     -- Building Renovation 
        ...
        when sfcs_green_activity_comgloval = 'Real estate - Building renovation loans' and concat(cproduto,csubprod) in ('096HGP','0960H8','096065') then 'Building Renovation'
        when sfcs_green_activity_comgloval = 'Real estate - Building renovation loans' and trim(nifrru)<>'' then 'Building Renovation'            
     ...
     -- Motor Vehicle Loans    
        ...
        when  idcomb_satelite like '%SC0304%' and des_combustivel = 'ELECTRICO' and cast(co2 as int) >= 50 then 'Motor Vehicle Loans'
        -- -	Gas and plug-in hybrid cars than pollute more than 50g CO2 per km
        when  idcomb_satelite like '%SC0304%' and (
                --(des_combustivel in ('HIBRIDO') and co2 > 50 and co2 <> '' and co2 is not null)  --- > De acordo com o email da Rute Ferreira recebido no dia 4/01 não existe informação nas tabelas de Lease que permita identificar plug-in
                --or
        ...
        then 'Motor Vehicle Loans'
    		...
    		when  idcomb_satelite like '%SC0304%' and sfcs_green_activity_comgloval like 'Transport%' then '' 
    		when  idcomb_satelite like '%SC0303%' and sfcs_green_activity_comgloval like 'Real%' then 'Real estate - other' 
...
insert overwrite table  bu_esg_work.RT_PILAR3_SATELITE83 partition (DT_RFRNC, PROC_ID)
...
left join
(Select nvl(max(PROC_ID),0)+1 as NEW_PROC_ID from bu_esg_work.RT_PILAR3_SATELITE83) RT
on 1=1
;
