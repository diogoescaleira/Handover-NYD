...
-- INSERT OVERWRITE TABLE BU_ESG_WORK.AG084_GAR_CTO_GRA PARTITION (ID_CORRIDA,REF_DATE)
...
FROM
    ...
    FROM
        ...
        and ID_CORRIDA IN (SELECT MAX(ID_CORRIDA) FROM BU_ESG_WORK.RF_PILAR3_UNIVERSO_FULL WHERE DT_RFRNC = '${REF_DATE}')
        and csatelite in (84) 
       ...
       SELECT
            ...
            CNUMECTA,
            ZDEPOSIT, 
            ccontab_final_cargabal AS CARGABAL_CT,
            
            -- COALESCE(V_CONTAB * -1,0) AS AMOUNT,

            CASE 
                WHEN PROPERTY_BRANCH_CODE IS NULL THEN MSALDO_FINAL
                ELSE V_CONTAB * -1
            END AS AMOUNT,
        ...
        (
			...
			(
				...
				FROM 
				(
					SELECT DISTINCT IDCOMB
					FROM BU_ESG_WORK.RF_PILAR3_SATELITES_IDCOMB_V2_JUN25
					WHERE CSATELITE=84 AND IDCOMB LIKE '%MC10%'
				) FULL_SAT84
				LEFT JOIN
				(
					SELECT
