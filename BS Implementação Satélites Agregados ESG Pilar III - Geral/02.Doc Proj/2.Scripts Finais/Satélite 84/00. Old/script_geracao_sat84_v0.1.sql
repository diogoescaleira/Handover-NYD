...
sfics.nome_ctgr_sfics,
   ...
   case 
        	when a.idcomb_satelite like '%MC08%' and  b.`33_counterparty_type` = 'instituicoes de credito' then 'INVS2'
        	when a.idcomb_satelite like '%MC08%' and  b.`33_counterparty_type` = 'setor publico' then 'INVS3'
        	when a.idcomb_satelite like '%MC08%' and  b.`33_counterparty_type` = 'outras empresas nao financeiras' then 'INVS5'
        	when a.idcomb_satelite like '%MC08%' and  b.`33_counterparty_type` = 'outras instituicoes financeiras' then 'INVS4'
        else '' end as investment_sector,


         case
            when a.idcomb_satelite like '%SC0301%' and b.`33_counterparty_type` = 'setor publico' and 
            	ct003.cnatureza_juri in ('121100','121210','121220','211110','211120','211130','221100','221210','121231') then'ESG2'  --- LISTA PARA DEFINIR SOVEREINGS PARTILHADA POR LUÍSA E DADA POR CAPITAL
            when a.idcomb_satelite like '%SC0301%' and b.`33_counterparty_type` = 'setor publico' then 'ESG1' 
            when a.idcomb_satelite like '%SC0301%' then 'ESG1'
            when a.idcomb_satelite like '%SC0302%' and a.`14_nace` in ('K65.3.0','K66.1.1','K64.3.0','K66.1.2','K66.3.0')    ---  MAPEAMENTO EM REVISÃO POR BUZÓN MAS MANTER PARA PILOTO JUN23
                and a.`33_counterparty_type` = 'outras instituicoes financeiras' then 'ESG3'
            when a.idcomb_satelite like '%SC0302%' and a.`14_nace` in ('K66.1.9','K64.2.0')   ---  MAPEAMENTO EM REVISÃO POR BUZÓN MAS MANTER PARA PILOTO JUN23
                and a.`33_counterparty_type` = 'outras instituicoes financeiras' then 'ESG4'          
            when a.idcomb_satelite like '%SC0302%' and  a.`14_nace` in ('K65.1.1','K65.1.2','K65.2.0', 'K66.2.1','K66.2.2','K66.2.9')    ---  MAPEAMENTO EM REVISÃO POR BUZÓN MAS MANTER PARA PILOTO JUN23
                and a.`33_counterparty_type` = 'outras instituicoes financeiras' then 'ESG5'          
            when a.idcomb_satelite like '%SC0302%' and b.`33_counterparty_type` = 'outras instituicoes financeiras' then 'ESG6'
            when a.idcomb_satelite like '%SC0302%' and b.`33_counterparty_type` = 'instituicoes de credito'  and a.`14_nace` in ('K64.1.1','K64.1.9','K64.9.1','K64.9.2','K64.9.9') then 'ESG6' ---  MAPEAMENTO EM REVISÃO POR BUZÓN MAS MANTER PARA PILOTO JUN23
            when a.idcomb_satelite like '%SC0302%' then 'ESG6'
            when (a.idcomb_satelite like '%MC08%' and b.`33_counterparty_type` = 'outras instituicoes financeiras') then 'ESG6'            
...
(select * from bu_esg_work.modesg_out_sfics_taxon_europeia ) sfics

on on  a.cempresa_ct = sfics.cempresa 
