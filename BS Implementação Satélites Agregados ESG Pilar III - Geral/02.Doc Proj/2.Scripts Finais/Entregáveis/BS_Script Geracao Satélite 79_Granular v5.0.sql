...
INSERT OVERWRITE TABLE BU_ESG_WORK.AG079_CESG_CTO_GRA PARTITION (REF_DATE)
...
FROM
    (
        ...
        (
            ...
            FROM
                ...
                (                
                    SELECT DISTINCT
                        CCONTAB_FINAL_IDCOMB_TOTAL AS IDCOMB_SATELITE,
                        CEMPRESA,
                        CBALCAO,
                        CNUMECTA,
                        ZDEPOSIT,
                        CCONTAB_FINAL_IDCOMB,
                        CCONTAB_FINAL_IFRS AS IFRS,
                        CONCAT(CEMPRESA, CBALCAO, CNUMECTA, ZDEPOSIT) AS CHAVE
                ...
                (
                SELECT CEMPRESA, CBALCAO, CNUMECTA, ZDEPOSIT, CEMPBEM, CKBALBEM, CKCTABEM, CKREFBEM, ZCLIENTE, SUM(AMOUNT) AS AMOUNT
        ...
        UNION ALL
            ...
            FROM
                ...
                (
    			SELECT  
    				A.CCONTAB_FINAL_IDCOMB_TOTAL AS IDCOMB_SATELITE,
    				A.CCONTAB_FINAL_IDCOMB,
    				A.CEMPRESA, 
    				A.CBALCAO,
    				A.CNUMECTA, 
    				A.ZDEPOSIT, 
    				A.CCONTAB_FINAL_IFRS AS IFRS,
    				CONCAT(A.CEMPRESA, A.CBALCAO, A.CNUMECTA, A.ZDEPOSIT) AS CHAVE,
    				ZCLIENTE,
    				SUM(A.MSALDO_FINAL) AS SALDO
    			FROM 
    			(
    				SELECT *
    				FROM BUSINESS_ESG.MODESG_OUT_REPORTE_GRANULAR
    			...
    			WHERE REF_DATE='${REF_DATE}'
    				AND NOME_PERIMETRO LIKE '%Individual Idcomb%'
                ) EMISS ON  A.CEMPRESA = EMISS.CEMPRESA
                            AND A.CBALCAO  = EMISS.CBALCAO
                            AND A.CNUMECTA = EMISS.CNUMECTA
                            AND A.ZDEPOSIT = EMISS.ZDEPOSIT
                GROUP BY 1,2,3,4,5,6,7,8,9
	...
	(
        ...
        ON UNIV.CEMPRESA=TRAD_CHAVES.CEMPRESA
		AND UNIV.CBALCAO=TRAD_CHAVES.CBALCAO
		AND UNIV.CNUMECTA=TRAD_CHAVES.CNUMECTA
		AND UNIV.ZDEPOSIT=TRAD_CHAVES.ZDEPOSIT
	...
	(
	    ...
	    ON TRAD_CHAVES.CEMPRESA_FR=FR012.CEMPRESA
		AND TRAD_CHAVES.CBALCAO_FR=FR012.CBALCAO
		AND TRAD_CHAVES.CNUMECTA_FR=FR012.CNUMECTA
		AND TRAD_CHAVES.ZDEPOSIT_FR=FR012.ZDEPOSIT
		AND UNIV.CCONTAB_FINAL_IDCOMB=FR012.CCONTAB_FINAL_IDCOMB
    ...
    (
        SELECT
            CEMPRESA, CBALCAO, CNUMECTA, ZDEPOSIT, DDVENCIM, ZCLIENTE
