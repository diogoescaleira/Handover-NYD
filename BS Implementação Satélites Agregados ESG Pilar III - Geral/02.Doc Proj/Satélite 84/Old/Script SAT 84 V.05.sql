...
SELECT
    ...
    case
        ...
        when a.idcomb_satelite like '%SC0302%' 
            and NFIN.COD_NACE_ESG in ('K65.3.0','K66.1.1','K64.3.0','K66.1.2','K66.3.0')    ---  campo nace passará a vir do universo full
            and b.`33_counterparty_type` = 'outras instituicoes financeiras' 
            then 'ESGS3'
        when a.idcomb_satelite like '%SC0302%' 
            and NFIN.COD_NACE_ESG in ('K66.1.9','K64.2.0')   ---  campo nace passará a vir do universo full
            and b.`33_counterparty_type` = 'outras instituicoes financeiras' 
            then 'ESGS4'
        when a.idcomb_satelite like '%SC0302%' 
            and  NFIN.COD_NACE_ESG in ('K65.1.1','K65.1.2','K65.2.0', 'K66.2.1','K66.2.2','K66.2.9')    ---  campo nace passará a vir do universo full
            and b.`33_counterparty_type` = 'outras instituicoes financeiras' 
            then 'ESGS5'
        when a.idcomb_satelite like '%SC0302%' 
            and b.`33_counterparty_type` = 'outras instituicoes financeiras' 
        ...
        when a.idcomb_satelite like '%SC0302%' 
            ...
            and NFIN.COD_NACE_ESG in ('K64.1.1','K64.1.9','K64.9.1','K64.9.2','K64.9.9') 
        ...
        CASE
			...
			WHEN A.IDCOMB_SATELITE LIKE '%MC13%' THEN ''
            WHEN A.IDCOMB_SATELITE LIKE '%SC0301%' AND B.`33_counterparty_type` = 'setor publico' AND CT003.CNATUREZA_JURI IN ('121100','121210','121220','211110','211120','211130','221100','221210','121231') THEN '' --LISTA PARA DEFINIR SOVEREINGS PARTILHADA POR LUÍSA E DADA POR CAPITAL
		...
		-- SPV
			-- WHEN (B.FLAG_SPV = '1') THEN 'PESG4'
			
		-- SFICS
			
			WHEN SFICS.NOME_CTGR_SFICS IN ('C.SLL', 'Pure Green counterparty - Default') THEN ''	
			WHEN (A.IDCOMB_SATELITE LIKE '%SC0301%' OR A.IDCOMB_SATELITE LIKE '%SC0303%') AND TRIM(SFICS.NOME_CTGR_SFICS) <> '' AND SFICS.NOME_CTGR_SFICS NOT IN ('C.SLL', 'Pure Green counterparty - Default') THEN 'PESG4'
            ELSE ''
        END AS PURPOSE_ESG,
        ...
        CASE
            ...
            WHEN CAST(B.`29_flag_specialised_lending` AS STRING) = '0' AND SFICS.NOME_PURPOSE_ESG_ALINH_TAX='Building renovation loans' THEN '' -- REGRA DEFINIDA COM BASE NAS COMBINAÇÕES POSSÍVEIS DE IDCOMBS
            WHEN CAST(B.`29_flag_specialised_lending` AS STRING) = '1' AND SFICS.NOME_SPECIFIC_ELIGIBLE='No' THEN 'SPLE2' -- FORÇADO COM BASE NO EMAIL DA CORPORAÇÃO DE DIA 09/01/2025
            WHEN CAST(B.`29_flag_specialised_lending` AS STRING) = '0' AND SFICS.NOME_GENERAL_SPECIFIC_PURPOSE='Specific purpose' THEN 'SPLE2' -- REGRA DEFINIDA COM BASE NAS COMBINAÇÕES POSSÍVEIS DE IDCOMBS
        ...
        --Turnover
		---Mitigation
        COALESCE(NFIN.CCM_TRNVR_TTL_ELGBL,0) AS TUCCM,
        COALESCE(NFIN.CCM_TRNVR_OWN_PRFRMNCE_ALGND,0) AS ETCCM,
        COALESCE(NFIN.CCM_TRNVR_TRNSTN_ALIGND,0) AS TRANT,
        COALESCE(NFIN.CCM_TRNVR_ENBLNG_ALGND,0) AS ENTCCM,
		---Adaptation
		COALESCE(NFIN.CCA_TRNVR_TTL_ELGBL,0) AS TUCCA,
        COALESCE(NFIN.CCA_TURNOVER_OWN_PRFRMNCE_ALGND,0) AS ETCCA,
        COALESCE(NFIN.CCA_TRNVR_ENBLNG_ALIGND,0) AS ENTCCA,
		--Water & Waste management
		COALESCE(NFIN.WTR_TRNVR_TTL_ELGBL,0) AS TUWTR,
		COALESCE(NFIN.WTR_TURNOVER_OWN_PRFRMNCE_ALGND,0) AS ETWTR,
		COALESCE(NFIN.WTR_TRNVR_ENBLNG_ALIGND,0) AS ENTWTR,
		---Circular Economy
		COALESCE(NFIN.CE_TRNVR_TTL_ELGBL,0) AS TUCE,
		COALESCE(NFIN.CE_TURNOVER_OWN_PRFRMNCE_ALGND,0) AS ETCE,
		COALESCE(NFIN.CE_TRNVR_ENBLNG_ALIGND,0) AS ENTCE,
		---Pollution
		COALESCE(NFIN.PPC_TRNVR_TTL_ELGBL,0) AS TUPPC,
		COALESCE(NFIN.PPC_TURNOVER_OWN_PRFRMNCE_ALGND,0) AS ETPPC,
		--Biodiversity
		COALESCE(NFIN.BIO_TRNVR_TTL_ELGBL,0) AS TUBIO,
		COALESCE(NFIN.BIO_TURNOVER_OWN_PRFRMNCE_ALGND,0) AS ETBIO,
		
		--Capex
		---Mitigation
	    COALESCE(NFIN.CCM_CAPEX_TTL_ELGBL,0) AS CACCM,
	    COALESCE(NFIN.CCM_CAPEX_OWN_PRFRMNCE_ALGND,0) AS ECCCM,
	    COALESCE(NFIN.CCM_CAPEX_TRNSTN_ALGND,0) AS TRANC,
	    COALESCE(NFIN.CCM_CAPEX_ENBLNG_ALGND,0) AS ENCCCM,
		---Adaptation 
	    COALESCE(NFIN.CCA_CAPEX_TTL_ELGBL,0) AS CACCA,
	    COALESCE(NFIN.CCA_CAPEX_OWN_PRFRMNCE_ALGND,0) AS ECCCA,
	    COALESCE(NFIN.CCA_CAPEX_ENBLNG_ALGND,0) AS ENCCCA,		
		---Water & Waste Management
		COALESCE(NFIN.WTR_CAPEX_TTL_ELGBL,0) AS CAWTR,
		COALESCE(NFIN.WTR_CAPEX_OWN_PRFRMNCE_ALGND,0) AS ECWTR,
		COALESCE(NFIN.WTR_CAPEX_ENBLNG_ALGND,0) AS ENCWTR,
		---Circular Economy
		COALESCE(NFIN.CE_CAPEX_TTL_ELGBL,0) AS CACE,
		COALESCE(NFIN.CE_CAPEX_OWN_PRFRMNCE_ALGND,0) AS ECCE,
		COALESCE(NFIN.CE_CAPEX_ENBLNG_ALGND,0) AS ENCCE,
		--Pollution
		COALESCE(NFIN.PPC_CAPEX_TTL_ELGBL,0) AS CAPPC,
		COALESCE(NFIN.PPC_CAPEX_OWN_PRFRMNCE_ALGND,0) AS ECPPC,
		--Biodiversity
		COALESCE(NFIN.BIO_CAPEX_TTL_ELGBL,0) AS CABIO,
		COALESCE(NFIN.BIO_CAPEX_OWN_PRFRMNCE_ALGND,0) AS ECBIO,
