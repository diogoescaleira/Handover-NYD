...
(
    ...
    (
        ...
        WHERE TEMPLATE5 = 1 AND
            ...
            AND BRUTO_IMPARIDADE = 'valor bruto' AND CONCAT(CEMPRESA,CBALCAO,CNUMECTA,ZDEPOSIT) NOT IN (SELECT CONCAT(CEMPRESA,CBALCAO,CNUMECTA,ZDEPOSIT) FROM CD_CAPTOOLS.CT001_UNIV_SALDO WHERE REF_DATE='${REF_DATE}' AND CCONTAB_FINAL_IDCOMB LIKE '%MC1005%')
    )UNI

    LEFT JOIN 
    (   
        SELECT CEMPRESA,CBALCAO,CNUMECTA,ZDEPOSIT,ZCLIENTE,CEMPBEM,CKBALBEM,CKCTABEM,CKREFBEM,
		SUM(CASE WHEN CONCAT(CEMPRESA,CBALCAO,CNUMECTA,ZDEPOSIT)='31000320631593096000000000000000' THEN AMOUNT*-1 ELSE AMOUNT END) AS AMOUNT
    ...
    (
        ...
        WHERE TEMPLATE5 = 1 AND
            REF_DATE = '${REF_DATE}' AND ID_CORRIDA IN (SELECT MAX(ID_CORRIDA) FROM BU_CAPTOOLS_WORK.P3_UNIV_LOCAL_CTO WHERE REF_DATE = '${REF_DATE}')
            AND BRUTO_IMPARIDADE = 'imparidade' AND CONCAT(CEMPRESA,CBALCAO,CNUMECTA,ZDEPOSIT) NOT IN (SELECT CONCAT(CEMPRESA,CBALCAO,CNUMECTA,ZDEPOSIT) FROM CD_CAPTOOLS.CT001_UNIV_SALDO WHERE REF_DATE='${REF_DATE}' AND CCONTAB_FINAL_IDCOMB LIKE '%MC1005%')
    )UNI
