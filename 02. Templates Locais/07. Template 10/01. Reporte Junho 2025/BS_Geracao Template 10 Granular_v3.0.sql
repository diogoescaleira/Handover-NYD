...
(
    SELECT 
    	...
    	F2.CKCTABEM,
    	F2.CKREFBEM,
    	CAST(CASE WHEN CONCAT(F1.CEMPRESA,F1.CBALCAO,F1.CNUMECTA,F1.ZDEPOSIT)='31000100200606273000000000000000' THEN F1.SALDO ELSE F2.SALDO END AS DECIMAL(38,6)) AS SALDO_CT
    FROM 
        (
    	SELECT CEMPRESA,CBALCAO,CNUMECTA,ZDEPOSIT,CONTRAPARTE,ZCLIENTE,INSTRUMENTO_FINANCEIRO,TIPO_COLATERAL,SUM(MSALDO_FINAL) AS SALDO
    ...
    LEFT JOIN 
        (
        SELECT CEMPRESA,CBALCAO,CNUMECTA,ZDEPOSIT,CEMPBEM,ckbalbem,ckctabem,ckrefbem,FLAG_BEM,SUM(AMOUNT) AS SALDO
        FROM BU_ESG_WORK.EMI_FINAN_V4
        WHERE FLAG_PERIMETRO LIKE '%ST SGPS Cons%'
            AND CONCAT(CEMPRESA,CBALCAO,CNUMECTA,ZDEPOSIT)<>'31000100200606273000000000000000'
